...
Node::~Node() {
  ...
  orb_slam_->SaveKeyFrameTrajectoryTUM(std::string(ROOT_DIR) + "/log/kf_traj.txt");
...
void Node::PublishPositionAsPoseStamped(cv::Mat position) {
  ...
  std::ofstream fout(std::string(ROOT_DIR) + "/log/traj.txt", std::ios_base::app);
  fout << std::fixed << std::setprecision(6)
       << current_frame_time_.toSec() << " "
       << std::setprecision(9)
       ...
       << pose_msg.pose.orientation.w << std::endl;
}
